  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-3 sm:p-6 md:p-8 font-sans">
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-5 sm:gap-6">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/60 border border-white/10 p-4 sm:p-5 rounded-3xl backdrop-blur-2xl">
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-emerald-500 to-purple-600 flex items-center justify-center text-xl sm:text-2xl shadow-lg shadow-emerald-500/20">
              ⛏️
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-base sm:text-xl font-bold tracking-tight">Minecraft Bedrock Inventory Matrix</h1>
                <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[9px] sm:text-[10px] uppercase font-mono px-2 py-0.5 rounded-full tracking-wider">
                  SECRET // v2.0
                </span>
              </div>
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 sm:py-2.5 rounded-2xl text-xs sm:text-sm font-medium border border-white/10 transition-all cursor-pointer"
            >
              <Upload size={15} />
            ...
            {world && (
              <button
                onClick={exportZip}
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-500 hover:brightness-110 text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-2xl text-xs sm:text-sm font-bold shadow-lg shadow-emerald-600/30 transition-all cursor-pointer"
              >
                <Download size={15} />
        ...
        {!world && (
          <div className="bg-slate-900/40 border border-white/10 rounded-3xl p-6 sm:p-8 text-center flex flex-col items-center justify-center gap-5 py-12 sm:py-16 backdrop-blur-xl">
            <div 
              onClick={() => fileInputRef.current?.click()}
              className="w-full max-w-xl border-2 border-dashed border-white/20 hover:border-emerald-400/60 rounded-3xl p-8 sm:p-10 flex flex-col items-center justify-center gap-4 bg-slate-900/30 hover:bg-slate-800/40 transition-all cursor-pointer group"
            >
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-2xl sm:text-3xl group-hover:scale-110 transition-transform">
                📂
              </div>
              <div>
                <h3 className="text-base sm:text-lg font-bold text-slate-100">Drop Bedrock World File Here</h3>
                <p className="text-xs sm:text-sm text-slate-400 mt-1">Supports iOS exports, .mcworld and .zip files</p>
              </div>
            </div>

            <div className="flex flex-wrap justify-center items-center gap-2 sm:gap-4 text-xs sm:text-sm text-slate-400">
        ...
        <div className="bg-slate-900/60 border border-white/10 p-3 sm:p-4 rounded-3xl flex flex-wrap items-center gap-2 sm:gap-3 backdrop-blur-xl">
          <span className="text-[11px] sm:text-xs font-mono uppercase text-slate-400 px-1 sm:px-2 flex items-center gap-1.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 border border-purple-500/30 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer"
          >
            ⚔️ Max Enchants (No Silk Touch)
          </button>
          <button
            onClick={maxEnchantGodArmor}
            className="flex items-center gap-1.5 bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 border border-blue-500/30 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer"
          >
            🛡️ God Armor
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6">
          <div className="lg:col-span-2 bg-slate-900/60 border border-white/10 p-4 sm:p-6 rounded-3xl flex flex-col gap-5 sm:gap-6 backdrop-blur-2xl">
            <div>
              <span className="text-xs font-mono uppercase text-slate-400 block mb-2.5">Equipment & Offhand</span>
            ...
            <div>
              <span className="text-xs font-mono uppercase text-slate-400 block mb-2.5">Storage Inventory (27 Slots)</span>
          ...
          <div className="bg-slate-900/60 border border-white/10 p-4 sm:p-6 rounded-3xl flex flex-col gap-5 backdrop-blur-2xl">
            <div className="flex items-center justify-between border-b border-white/10 pb-3 sm:pb-4">
              <div>
                <h3 className="font-bold text-sm sm:text-base text-slate-100">Slot Inspector</h3>
            ...
            {selectedSlot ? (
              <div className="flex flex-col gap-4">
                <button
                  ...
                  className="w-full flex items-center justify-between p-3 rounded-2xl bg-slate-800/80 hover:bg-slate-800 border border-white/10 transition-all cursor-pointer group"
                >
                  ...
                  <Search size={15} className="text-slate-400 group-hover:text-white transition-colors" />
                </button>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[11px] font-mono uppercase text-slate-400 block mb-1">Count</label>
                    <input
                      type="number"
                      min={1}
                      max={127}
                      ...
                      className="w-full bg-slate-950/60 border border-white/10 rounded-xl px-3 py-1.5 text-xs sm:text-sm text-slate-100 focus:outline-none focus:border-emerald-400 font-mono"
                    />
                  </div>
                  <div>
                    <label className="text-[11px] font-mono uppercase text-slate-400 block mb-1">Damage</label>
                    <input
                      type="number"
                      min={0}
                      value={editorDamage}
                ...
                <div>
                  <label className="text-[11px] font-mono uppercase text-slate-400 block mb-1">Custom Name</label>
                  <input
                    type="text"
                    placeholder="e.g. Legendary Pickaxe"
                    value={editorCustomName}
                    onChange={(e) => setEditorCustomName(e.target.value)}
                    className="w-full bg-slate-950/60 border border-white/10 rounded-xl px-3 py-1.5 text-xs sm:text-sm text-slate-100 focus:outline-none focus:border-emerald-400"
                  />
                </div>

                <div className="flex flex-col gap-2.5">
                  <label className="text-[11px] font-mono uppercase text-slate-400">Enchantments ({editorEnchantments.length})</label>
                  ...
                  <select
                    ...
                    defaultValue=""
                    className="w-full bg-slate-950/60 border border-white/10 rounded-xl px-2.5 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-purple-400 cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 rounded-2xl text-xs sm:text-sm transition-all shadow-lg shadow-emerald-600/30 cursor-pointer mt-1"
                >
                  ✓ Apply Changes to Slot
                </button>
              </div>
            ) : (
              <div className="text-center text-slate-500 text-xs sm:text-sm py-10">
            ...
            >
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-base sm:text-lg text-slate-100">Select Item</h3>
                </div>
                <button 
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-white/10 rounded-2xl pl-9 pr-4 py-2 text-xs sm:text-sm text-slate-100 focus:outline-none focus:border-emerald-400"
                  autoFocus
                />
              </div>
                ...
                {filteredPickerItems.map(item => (
                  ...
                  >
                    <img 
                      ...
                      onError={(e) => { (e.target as HTMLElement).style.display = 'none'; }}
                    />
                    <span className="text-[10px] sm:text-[11px] font-medium text-slate-300 line-clamp-1">
                      {item.name}
                    </span>
                  </button>
                ))}
